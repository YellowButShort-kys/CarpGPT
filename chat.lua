...
function base:GetOpenAIContents()
    ...
    for _, var in ipairs(self:GetRawContents()) do
        table.insert(out, {role = var.role, content = (var.content:gsub("{{user}}", "User"):gsub("{{char}}", "Assistant"))})
    end
    return out
end
function base:GetInstructContents()
    ...
    table.insert(out, {role = "assistant", content="Assistant:"})
    return out
end
...
function base:GetResponse(chat, msg, user, callback, errcallback)
    ...
    elseif model == "dolphin" then
        self.task = dolphin.Generate(self:GetInstructContents(), callback, errcallback, {chat, self, msg, user}, {"Assistant:", "User:"})
    elseif model == "soliloque" then
        self.task = soliloque.Generate(self:GetInstructContents(), callback, errcallback, {chat, self, msg, user}, {"Assistant:", "User:"})
    elseif model == "llama8" then
        self.task = llama8.Generate(self:GetInstructContents(), callback, errcallback, {chat, self, msg, user}, {"Assistant:", "User:"})
    end
end
